// fetch.go 中的函数用来构造各种类型的数据库字段的填充行为
// 
...
// 
// Int（实际是int64，但是不隐式兼容=_=）, Uint（实际是Uint64不解释），Float
//（实际是 Float64 不解释）， Bool，String。所以这里弄太复杂也是白瞎。等以后玩熟
// bytes包再说吧。有兴趣的朋友可以看一下
...
// 
// 原想给了 NullXxxx 类型的可Scan的数据库类型字段的，可以提供填充，
// 后来一想调用者如果都自己用这一套了
// 也就没 ORM 什么必要了吧……
//
// fetch 目前都是私有的，但是其实如果后面我们发现在pgears之外也有足够的需求，这些函数完全有可能
// 导出为公开的。
package pgears
